...
Nous avons d'abord généré aléatoirement des graphes ayant entre 10000 et 100000 sommets (avec un pas de 1000), de degré sortant aléatoire compris entre 1 et 3, puis nous avons utilisé \lstbash{./bench.sh} pour générer la figure \ref{fig:graph} comparant les temps d'exécution de l'algorithme naïf et de l'algorithme des tas de Fibonacci.
  ...
  \begin{center}
    \begin{tikzpicture}
      \begin{axis}[
        ...
        \addplot table[x=nbVert,y=naive,col sep=tab] {bench.dat} node[above, pos=1] {Naïf};
        \addplot table[x=nbVert,y=opti,col sep=tab] {bench.dat} node[above, pos=1] {Fibonacci};
        \end{axis}
    \end{tikzpicture}
